...
- `/dashboard/` — 슈퍼관리자: 홈, 교육생, 출결, 테스트, 교육일지, 실습일지, 리포트, 심화교육 종합, 공지사항, 기수 관리
- `/manager/` — 매장관리자: 홈(성장추이), R&P 평가, 교육 총평
- `/my/` — 교육생: 홈(+공지 팝업), 공지사항, 출결, 테스트, 교육일지, 실습일지, 심화교육(준비중)
...
- **2컬럼 메인**: 왼쪽(차시별 평균, 추이 차트, 주의 교육생) / 오른쪽(교육일지 제출현황, 실습일지 실적, 최근 공지, 교육생 답글)
