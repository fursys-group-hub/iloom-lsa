...
- [x] **교육 인사이트 페이지 전면 리빌드 (mock 제거 + HR 관점 재구성)**
  - 가상 수주 데이터 완전 제거 → 실습일지 + 주간수주 엑셀 기반 진짜 데이터로 교체
  - 페이지 목적 재정의: "영업 대시보드" 아닌 **"교육 효과성 증명"** (Q1. 효과 있었나? + Q3. 다음 기수 개선?)
  - 위험 교육생 발견 섹션 제외 (교육생 분석 페이지와 중복이라)
  - 7개 섹션 재구성: KPI → 자기효능감 콤보 → 자신감/참여도 → 시험/적응 → 분야별/키워드 → 현장성과 → 교차분석+업로드
- [x] **인포그래픽 헤드라인 패턴 도입** (WeTransfer/Tmap 레퍼런스 적용)
  - 각 카드 구조: [큰 헤드라인 — 사람 말투 3줄] + [작은 데이터 근거] + [차트] + [하이라이트]
  - 헤드라인 예: "교육 후 / 고객 응대 자신감이 / 가장 많이 자랐어요" (22px / 800 / line-height 1.2)
  - 색상 강조: 핵심 키워드만 컬러로 (green/red)
  - 데이터 숫자는 보조로 작게 (13px / muted)
- [x] **"왜 보나요?" Popover 재사용 컴포넌트**
  - 전 카드(KPI, 자기효능감, 만족도, 자신감/참여도, 시험, 적응, 분야별, 키워드, 현장성과, 산점도)에 일관 적용
  - popover 내용 구성: 측정 의미 → 측정 방식 → 영역 설명 → 활용 방법 → 커크패트릭 매핑
  - HR 담당자/비개발자도 맥락을 이해할 수 있도록 상세하게 (5~10줄)
  - `summary` 스타일: 12px / 400 / muted (보조 액션, 디자인 시스템 "전체보기 →"와 동일 계층)
- [x] **HR 측정 프레임워크 커크패트릭 4단계 매핑**
  - 1단계 반응 → 교육 만족도 설문
  - 2단계 학습 → 시험 성적 + 적응 지수
  - 3단계 행동 → 자기효능감 사전-사후 + 관리자 R&P 평가
  - 4단계 결과 → 주간 수주 데이터
- [x] **자기효능감 사전-사후 설문 시스템**
  - 교육생 설문 페이지 신규 (`/my/survey`) — 자신감 5문항 + 만족도 3문항 + 주관식 3문항
  - 2회 실시: 입문교육 마지막 날(사전) + 심화교육 마지막 날(사후)
  - **HR 전문 용어 숨김**: "자기효능감" → "지금 나의 자신감은?" (설문 의도 노출 방지 → 솔직한 답변 유도)
  - 5점 척도 버튼: `radius-sm`(8px) — 너무 동그란 pill 지양
- [x] **차트 다양화 (HR 친화적)**
  - 레이더(polygon) — 자기효능감 5영역 사전/사후 겹침
  - 산점도(ScatterChart) — 시험 성적 vs 수주 전환율 + Pearson 상관계수 자동 계산
  - 멀티라인 밴드 — 교육생 5명 주차별 수주 성장 비교
  - 스택바 — 자신감 분포(높음/보통/낮음) 추이
  - 교육생 분석 페이지의 깔끔한 레이더 패턴 재활용(polygon + 표 병렬)
- [x] **콤보 카드 구조 (한 설문 데이터 묶기)**
  - 자기효능감 + 만족도 + 목소리를 **하나의 카드** 안에 좌:우 4:6 분할 (같은 설문 출처이므로)
  - 가운데 세로 구분선 + 모바일에서 가로 border-top으로 자동 변환
  - 우측 "교육생 목소리"는 성장/걱정 grid 2열 (답변 박스 제거, 구분선만 있는 깔끔한 리스트)
- [x] **KPI 4카드 한 컨테이너 묶기**
  - 4개 별도 카드 → 하나의 카드 안에 4칸 (교육일지 페이지 stat row 패턴)
  - 숫자 아래 판정 뱃지 추가 (우수/보통/보강 필요/양호/주의/위험/큰 성장/성장/정체/미실시)
- [x] **모바일 반응형 정리 (1023px / 768px 2단계)**
  - 1023px 이하: 콤보 카드 1열 stack (구분선이 자동으로 상하 전환)
  - 1023px 이하: 교육생 목소리 grid → 1열 (성장 → 걱정 순서)
  - 768px 이하: KPI 4개 → 2x2 격자, 모든 2열 그리드 → 1열
- [x] **교육생 상세 "성장 여정" 섹션 (IDP)**
  - 자기효능감 미니 바 차트 + 자신감 타임라인(이모지) + 참여도 변화 + 주관식 하이라이트
